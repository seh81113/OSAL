...
/* POWER is at P1.5 */
#define HAL_KEY_POWER_PORT   P1
#define HAL_KEY_POWER_BIT    BV(5)
#define HAL_KEY_POWER_SEL    P1SEL
#define HAL_KEY_POWER_DIR    P1DIR

/* edge interrupt */
#define HAL_KEY_POWER_EDGEBIT  BV(3)
#define HAL_KEY_POWER_EDGE     AL_KEY_RISING_EDGE//HAL_KEY_FALLING_EDGE//

/* POWER interrupts */
#define HAL_KEY_POWER_IEN      IEN2  /* CPU interrupt mask register */
#define HAL_KEY_POWER_IENBIT   BV(4) /* Mask bit for all of Port_0 */
#define HAL_KEY_POWER_ICTL     P1IEN /* Port Interrupt Control register */
#define HAL_KEY_POWER_ICTLBIT  BV(5) /* P0IEN - P0.1 enable/disable bit */
#define HAL_KEY_POWER_PXIFG    P1IFG /* Interrupt flag at source */
...
#else
  HAL_KEY_POWER_SEL &= ~(HAL_KEY_POWER_BIT);    /* Set pin function to GPIO */
  HAL_KEY_POWER_DIR &= ~(HAL_KEY_POWER_BIT);    /* Set pin direction to Input */
  HAL_KEY_JOY_MOVE_SEL &= ~(HAL_KEY_JOY_MOVE_BIT); /* Set pin function to GPIO */
  HAL_KEY_JOY_MOVE_DIR &= ~(HAL_KEY_JOY_MOVE_BIT); /* Set pin direction to Input */

  P2INP |= PUSH2_BV;  /* Configure GPIO tri-state. */  //p2口设置为3态
#endif
...
void HalKeyConfig (bool interruptEnable, halKeyCBack_t cback)
{
  ...
  if (Hal_KeyIntEnable)
  {
...
#else
    /* Rising/Falling edge configuratinn */
    PICTL &= ~(HAL_KEY_POWER_EDGEBIT);    /* Clear the edge bit */
    /* For falling edge, the bit must be set. */
  #if (HAL_KEY_POWER_EDGE == HAL_KEY_FALLING_EDGE)
    PICTL |= HAL_KEY_POWER_EDGEBIT;
  #endif
     ...
     */
    HAL_KEY_POWER_ICTL |= HAL_KEY_POWER_ICTLBIT;
    HAL_KEY_POWER_IEN |= HAL_KEY_POWER_IENBIT;
    HAL_KEY_POWER_PXIFG = ~(HAL_KEY_POWER_BIT);
...
#else
    HAL_KEY_POWER_ICTL &= ~(HAL_KEY_POWER_ICTLBIT); /* don't generate interrupt */
    HAL_KEY_POWER_IEN &= ~(HAL_KEY_POWER_IENBIT);   /* Clear interrupt enable bit */
#endif  // !CC2540_MINIDK
...
uint8 HalKeyRead ( void )
{
  ...
  if ( (HAL_KEY_POWER_PORT & HAL_KEY_POWER_BIT))    /* Key is active high */
#elif defined (HAL_BOARD_CC2530EB_REV13)
  if (!(HAL_KEY_POWER_PORT & HAL_KEY_POWER_BIT))    /* Key is active low */
#endif
  {
    keys |= HAL_KEY_POWER;
  }
...
#else
  if (HAL_KEY_POWER_PXIFG & HAL_KEY_POWER_BIT)
#endif
  {
