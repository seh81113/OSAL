...
void Lcd_Pixel(unsigned char x, unsigned char y, unsigned char columnLen, unsigned int ByteCount, unsigned char BMP[])
{
    ...
    Write_IIC_Command(0x40 + y & 0x1F);     // Set display startline
                                    //
    Write_IIC_Command(0x21);                // Set column Address
    Write_IIC_Command(y & 0x3F);
    Write_IIC_Command(y & 0x3F + columnLen * 8);

    unsigned int i = 0;
    for (i = 0; i < ByteCount; i++) {
      Write_IIC_Byte(BMP[i]);
...
void Write_IIC_Byte(unsigned char IIC_Byte)
{
	...
	for(i=0;i<8;i++)		
	{
		...
		m=m&0x80;
		if(m==0x80)
		{OLED_SDIN_Set();}//{OLED_SDIN_Clr();}//
		else OLED_SDIN_Clr();//OLED_SDIN_Set();//
			da=da<<1;
		OLED_SCLK_Set();
		OLED_SCLK_Clr();
		}
...
{         	
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				//OLED_ShowChar(x+(size2/2)*t,y,' ',size2);//多余的0不做显示
				continue;
			}else enshow=1; 
